...
```{r, echo=F}
d <- fread('https://crossley.github.io/book_stats/data/maze/d_long.csv')
d
```
...
```{r, echo=F}
d <- fread('https://crossley.github.io/book_stats/data/maze/d_wide.csv')
d
```

* Each format has its pros and cons, and you may find
  yourself needing to switch between them even within the
  same dataset.
...
* Using these functions is just a bit beyond the scope of
  this lecture, but we will return later in the unit to
  learn about them. If you are feeling confident in your use
  of `data.table` then you may enjoy getting a little ahead
  of the game by reading the following vignette:
