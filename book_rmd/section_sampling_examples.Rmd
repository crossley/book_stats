...
y <- c(0.1, 0.15, 0.25, 0.25, 0.15, 0.1)
...
variables a good way to do this is to use a simple bar plot.

```{r, echo=F}
# plot a bar graph to count samples
...
We can see that the most common outcomes are 2 and 3, which
is what we would expect given that the peak of the
probability distribution occurs at this value, but we also
get many other outcomes. In general, the count of
occurrences of each outcomee is about proportional to the
height of the bar in the probability distribution. This is
the essence of the concept of a probability distribution.
